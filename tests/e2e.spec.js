...
test.describe("Gatsby Blog E2E Tests", () => {
  ...
  test("can navigate to blog page from homepage", async ({ page }) => {
    await page.goto("/")
    const blogLink = page
      .locator('a[href*="/blog"], a:has-text("Blog")')
      .first()
  ...
  test("tags page displays tag cards", async ({ page }) => {
    ...
    await expect(
      page.locator('h1:has-text("Tags"), h1:has-text("#Tags")')
    ).toBeVisible()
    await page.waitForSelector(".ant-col", { timeout: 10000 })
  ...
  test("can navigate to tags from homepage", async ({ page }) => {
    await page.goto("/")
    const tagsLink = page
      .locator('a[href*="/tags"], a:has-text("Tags")')
      .first()
  ...
  test("contact form validates required description field", async ({
    page,
  }) => {
    await page.goto("/contact")
